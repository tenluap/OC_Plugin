...
8. **Create on Request**: For each approved agent, fetch the skill's SKILL.md (`skills_sh_detail`, or the raw GitHub fallback) and write `.opencode/agents/<agent-name>.md` with front matter `description`, `mode`, and — when scoping matters — `permission` entries in the current schema: keys are tool names (`edit`, `bash`, `webfetch`, `read`, `grep`, `glob`, `task`, `skill`, `websearch`, `question`, `external_directory`, `doom_loop`, or `*`), values are only `allow`, `ask`, or `deny` (string form; e.g. a read-only reviewer gets `permission: {edit: deny, bash: deny}`). **Never write a `tools:` boolean map** (legacy, deprecated) and **never a `model` field** — omitting `model` makes the agent run on the user's default OpenCode model; do not copy any `model:` hint from the source skill. The body carries over the skill's instructions faithfully — adapted to OpenCode tool names, but do not invent new behaviors the skill doesn't describe. Record provenance in the body's first line: `<!-- generated from skills.sh <owner/repo/slug> on <date> -->`.
...
- Agents must follow OpenCode agent markdown conventions: YAML front matter (`description` required; `mode`, `temperature`, `permission` optional; `permission` values only `allow`/`ask`/`deny` keyed by tool name — **never the legacy `tools:` boolean map**, and **never set `model`** — leaving it out selects the user's default OpenCode model), body = system prompt, filename = agent name in lowercase-hyphen form.
